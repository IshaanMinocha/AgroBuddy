  ...
  const processVoiceQuery = async ()=>{
    try{
      ...
      const processResponse = await axios.post(
        `${BACKEND_URL}/api/voicebot/process`,
        {
      ...
      const botAnswer = processResponse.data.answer;
      console.log('Got answer:', botAnswer);
      setAnswer(botAnswer);
  
      // 4. Speak the answer directly
      const ttsResponse = await axios.post(
        `${BACKEND_URL}/api/voicebot/in`,
        {
          text: botAnswer, // Use the answer directly
        },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
